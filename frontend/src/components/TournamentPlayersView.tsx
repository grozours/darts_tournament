...
function TournamentPlayersView() {
  ...
  const tournamentId = globalThis.window
    ? new URLSearchParams(globalThis.window.location.search).get('tournamentId')
    : null;
  ...
  const getSafeAccessToken = useCallback(async (): Promise<string | undefined> => {
    ...
    try {
      ...
      console.warn('[TournamentPlayersView] Failed to get access token:', err);
      return undefined;
    }
  ...
  const fetchTournamentDetails = useCallback(async (id: string) => {
    try {
      ...
      console.error('[TournamentPlayersView] Error fetching tournament:', err);
    }
  }, [getSafeAccessToken]);
  ...
  const fetchPlayers = useCallback(async (id: string) => {
    ...
    try {
      ...
      console.error('[TournamentPlayersView] Error fetching players:', err);
