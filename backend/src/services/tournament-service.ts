...
export class TournamentService {
  ...
  private isSafeEmailFormat(email: string): boolean {
  ...
  private validatePlayerData(data: CreatePlayerRequest): void {
    ...
    if (data.email && !this.isSafeEmailFormat(data.email)) {
      throw new AppError(
  ...
  private async reconcileTargetAvailability(
    ...
    for (const target of targets) {
      ...
      if (!target.currentMatchId) {
        await this.tournamentModel.setTargetAvailable(target.id);
      ...
      await this.tournamentModel.setTargetAvailable(target.id, completedAt);
      target.status = TargetStatus.AVAILABLE;
      // eslint-disable-next-line unicorn/no-null
      target.currentMatchId = null;
    }
  ...
  private buildPlayerLabel(player?: { firstName?: string; lastName?: string }) {
  ...
  private buildPoolStandings(pool: Awaited<ReturnType<TournamentModel['getPoolsWithMatchesForStage']>>[number]) {
    ...
    this.addPoolAssignments(rows, pool.assignments ?? []);
    this.addPoolMatchResults(rows, pool.matches ?? []);

    return this.sortPoolStandings(rows);
  }

  private ensurePoolRow(
  ...
  private addPoolAssignments(
    rows: Map<string, { playerId: string; name: string; legsWon: number; legsLost: number }>,
    assignments: Array<{ player?: { id?: string; firstName?: string; lastName?: string } }>
  ): void {
    for (const assignment of assignments) {
      this.ensurePoolRow(rows, assignment.player);
    }
  }

  private addPoolMatchResults(
    rows: Map<string, { playerId: string; name: string; legsWon: number; legsLost: number }>,
    ...
    for (const match of matches) {
      ...
      for (const pm of playerMatches) {
        this.ensurePoolRow(rows, pm.player);
      }
      this.applyPoolMatchScores(rows, playerMatches);
    }
  }

  private applyPoolMatchScores(
    rows: Map<string, { playerId: string; name: string; legsWon: number; legsLost: number }>,
    ...
    for (const pm of playerMatches) {
      ...
      row.legsLost += this.sumOpponentLegs(playerMatches, pm.player.id);
    }
  }

  private sumOpponentLegs(
    playerMatches: Array<{ player?: { id?: string }; scoreTotal?: number | null; legsWon?: number | null }>,
    playerId: string
  ): number {
    let opponentTotal = 0;
  ...
  private async populateBracketsForStage(tournamentId: string, stageId: string): Promise<void> {
    const stage = await this.getFinalPoolStage(tournamentId, stageId);
    if (!stage) {
      return;
    }
    ...
    const { winners, losers } = this.collectPoolStandings(pools, stage.advanceCount);
    const { winnerEntries, loserEntries } = this.buildBracketEntriesFromPools(winners, losers);
    const { winnerBracket, loserBracket } = this.selectWinnerLoserBrackets(brackets);
  ...
  private async getFinalPoolStage(tournamentId: string, stageId: string) {
  ...
  private uniqueEntriesByPlayerId<T extends { playerId: string }>(entries: T[]): T[] {
  ...
  private buildBracketEntriesFromPools(winners: PoolStandingsEntry[], losers: PoolStandingsEntry[]) {
    const winnerList = this.uniqueEntriesByPlayerId([...winners].sort(compareByPoolAndPosition));
    const loserList = this.uniqueEntriesByPlayerId([...losers].sort(compareByPoolAndPosition));

    const totalEntries = winnerList.length + loserList.length;
    const targetWinnerCount = this.computeTargetWinnerCount(winnerList.length, totalEntries);
    const rankedLosers = this.rankLosers(loserList);
  ...
  private computeTargetWinnerCount(winnerCount: number, totalEntries: number): number {
  ...
  private rankLosers(entries: PoolStandingsEntry[]): PoolStandingsEntry[] {
  ...
  private selectWinnerLoserBrackets(brackets: Awaited<ReturnType<TournamentModel['getBrackets']>>) {
  ...
  async updateTournamentPlayer(
    ...
    this.ensureTournamentAllowsPlayerUpdate(tournament);

    const player = await this.tournamentModel.getPlayerById(playerId);
    if (player?.personId) {
      await this.updateLinkedPerson(player.personId, updateData);
    }

    await this.ensureUniquePlayerAttributes(tournament, tournamentId, updateData, playerId);

    const playerUpdate = this.buildPlayerUpdate(updateData, player?.personId);
    return await this.tournamentModel.updatePlayer(tournamentId, playerId, playerUpdate);
  }

  private ensureTournamentAllowsPlayerUpdate(tournament: Tournament): void {
  ...
  private async updateLinkedPerson(personId: string, updateData: CreatePlayerRequest): Promise<void> {
    const personUpdate = {
    ...
    await this.tournamentModel.updatePerson(personId, personUpdate);
  }

  private async ensureUniquePlayerAttributes(
    tournament: Tournament,
    tournamentId: string,
    updateData: CreatePlayerRequest,
    playerId: string
    ...
    const needsUniqueTeam = tournament.format === TournamentFormat.DOUBLE
      || tournament.format === TournamentFormat.TEAM_4_PLAYER;
  ...
  private buildPlayerUpdate(updateData: CreatePlayerRequest, personId?: string | null) {
    return {
      ...(personId ? { personId } : {}),
      firstName: updateData.firstName.trim(),
      lastName: updateData.lastName.trim(),
  ...
  async completePoolStageWithRandomScores(
    ...
    const now = new Date();
    for (const match of matches) {
      await this.completeMatchWithRandomScores(match, now, { shouldAdvance: false });
    }
  ...
  async completeBracketRoundWithRandomScores(
    ...
    const now = new Date();
    for (const match of matches) {
      await this.completeMatchWithRandomScores(match, now, { shouldAdvance: true, tournamentId });
    }
  }

  private getMatchPlayerIds(match: { playerMatches?: Array<{ playerId?: string | null }> | null }): string[] {
    return (match.playerMatches || [])
      .map((pm) => pm.playerId)
      .filter(Boolean) as string[];
  }

  private buildRandomMatchScores(firstId: string, secondId: string) {
  ...
  private async completeMatchWithRandomScores(
    match: {
      id: string;
      status?: string | null;
    ...
    const playerIds = this.getMatchPlayerIds(match);
    ...
    const { scores, winnerId } = this.buildRandomMatchScores(firstId, secondId);
    ...
    if (options.shouldAdvance && options.tournamentId) {
      await this.advanceBracketIfReady(match.id, options.tournamentId);
    }
  }
  ...
  async updateMatchStatus(
    ...
    if (status === MatchStatus.COMPLETED || status === MatchStatus.CANCELLED) {
      await this.finalizeMatchStatus(matchId, match, status, now);
      return;
    }

    await this.tournamentModel.updateMatchStatus(matchId, status, {});
  }

  private async startMatchWithTarget(
    tournament: Tournament,
    matchId: string,
    ...
    await this.tournamentModel.startMatchWithTarget(matchId, targetToUse, now);
    await this.emitMatchStartedNotification(tournament, matchId);
  }

  private async finalizeMatchStatus(
    matchId: string,
    match: NonNullable<Awaited<ReturnType<TournamentModel['getMatchById']>>>,
    status: MatchStatus,
    now: Date
  ): Promise<void> {
    const timestamps = this.buildMatchCompletionTimestamps(match, status, now);
    if (match.targetId) {
      await this.tournamentModel.finishMatchAndReleaseTarget(
        matchId,
        match.targetId,
  ...
  private buildMatchCompletionTimestamps(
    match: NonNullable<Awaited<ReturnType<TournamentModel['getMatchById']>>>,
    status: MatchStatus,
    now: Date
  ...
  private async ensureTargetForMatchStart(
    ...
    const targetToUse = this.resolveTargetSelection(match, targetId);
    const target = await this.tournamentModel.getTargetById(targetToUse);
    this.ensureTargetExistsForTournament(target, tournamentId);
    await this.ensureTargetAvailability(target);
    return targetToUse;
  }
  ...
  private ensureTargetExistsForTournament(
  ...
  private async ensureTargetAvailability(
    target: Awaited<ReturnType<TournamentModel['getTargetById']>>
  ): Promise<void> {
    if (!target) {
      throw new AppError('Target not found', 404, 'TARGET_NOT_FOUND');
  ...
  private async releaseStaleTargetUsage(
    // eslint-disable-next-line unicorn/no-null
  ...
  private async advanceBracketIfReady(matchId: string, tournamentId: string): Promise<void> {
    ...
    const totalRounds = await this.ensureBracketTotalRounds(bracket);
    const roundNumber = match.roundNumber || 1;

    if (await this.tryCompleteFinalRound(bracket, roundNumber, totalRounds, tournamentId)) {
      return;
    }
    ...
    await this.advanceWinnerToNextRound(bracket, match, winnerId, roundNumber, tournamentId);
  }

  private async ensureBracketTotalRounds(
  ...
  private async tryCompleteFinalRound(
    bracket: NonNullable<Awaited<ReturnType<TournamentModel['getBracketById']>>>,
    roundNumber: number,
    totalRounds: number,
    tournamentId: string
  ...
  private async advanceWinnerToNextRound(
    ...
    if (sibling?.status !== MatchStatus.COMPLETED || !sibling?.winnerId) {
      await this.applySingleWinnerAdvance(
        tournamentId,
        bracket.id,
        existingNextMatch,
        nextRound,
  ...
  private async applySingleWinnerAdvance(
    tournamentId: string,
    bracketId: string,
    ...
    if (existingNextMatch) {
      ...
      await this.setBracketInProgress(bracketId);
      return;
    }
  ...
  private async applyPairAdvance(
    tournamentId: string,
    bracketId: string,
    existingNextMatch: Awaited<ReturnType<TournamentModel['getBracketMatchesByRoundWithPlayers']>>[number] | undefined,
    nextRound: number,
    ...
    if (existingNextMatch) {
      if (existingNextMatch.status === MatchStatus.SCHEDULED && (existingNextMatch.playerMatches?.length ?? 0) < 2) {
        await this.tournamentModel.setBracketMatchPlayers(existingNextMatch.id, playerIds);
      }
      await this.setBracketInProgress(bracketId);
      return;
    }

    await this.tournamentModel.createBracketMatches(tournamentId, bracketId, [
      {
  ...
  private async setBracketInProgress(bracketId: string): Promise<void> {
